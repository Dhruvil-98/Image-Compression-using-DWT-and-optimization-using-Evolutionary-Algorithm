...
function [outfreq] = geneticalgo(num,hi,lamda,gamma,dh,sm)
...
[u,fval,NumEval]=ffa_mincon(@fitnessfuncsc1,u0,Lb,Ub,para,hi,lamda,gamma,dh,sm);
...
% Performing selection from population of chromosomes
function [nbest,fbest,NumEval]...
           =ffa_mincon(fhandle,u0, Lb, Ub, para,hi,lamda,gamma,dh,sm)
...
% Evaluate new solutions (for all n)
for i=1:n,
   zn(i)=fitnessfuncsc1(ns(i,:),hi,lamda,gamma,dh,sm);
   Lightn(i)=zn(i);
end

% Ranking individual among large population by their light intensity/objectives
...
% Crossover of selected chromosomes
...
% This function is optional, as it is not in the original GA
...
% updating the final new generation of chromosomes
